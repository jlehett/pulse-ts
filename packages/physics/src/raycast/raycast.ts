...
export function raycast(colliders: Iterable<Collider>, origin: Vec3, dir: Vec3, maxDist = Infinity, filter?: (c: Collider) => boolean): RaycastHit | null {
    ...
    for (const c of colliders) {
        ...
        const off = Quat.rotateVector(rot, new Vec3(c.offset.x, c.offset.y, c.offset.z));
        const center = new Vec3(trs.position.x + off.x, trs.position.y + off.y, trs.position.z + off.z);
        let hitDist: number | null = null;
        let n = { x: 0, y: 0, z: 0 };
        if (c.kind === 'sphere') {
            const lx = ox - center.x, ly = oy - center.y, lz = oz - center.z;
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (t1 > tmin) { tmin = t1; hitAxis = i * sign; }
        ...
        if (hitDist != null && (!best || hitDist < best.dist)) best = { dist: hitDist, node: c.owner, nx: n.x, ny: n.y, nz: n.z };
    }
    if (!best) return null;
    return {
        node: best.node,
