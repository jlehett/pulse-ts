...
export function raycast(
    ...
    for (const c of colliders) {
        ...
        if (c.kind === 'sphere') {
            ...
            // Ray-plane
            const pn = Quat.rotateVector(trs.rotation, c.planeNormal.clone().normalize());
            const off = Quat.rotateVector(trs.rotation, c.offset.clone());
            const point = new Vec3(trs.position.x + off.x, trs.position.y + off.y, trs.position.z + off.z);
            const denom = pn.x * dx + pn.y * dy + pn.z * dz;
            if (Math.abs(denom) > 1e-8) {
                const tHit = ((point.x - ox) * pn.x + (point.y - oy) * pn.y + (point.z - oz) * pn.z) / denom;
                if (tHit >= 0 && tHit <= maxDist) {
                    hitDist = tHit;
            ...
            const r = c.capRadius;
            // Ray-sphere helper
            const raySphere = (cx: number, cy: number, cz: number, rr: number) => {
            ...
            const projM = ((ox - a.x) * abx + (oy - a.y) * aby + (oz - a.z) * abz) / ab2;
            const ndx = dx - projN * abx, ndy = dy - projN * aby, ndz = dz - projN * abz;
            const mdx = ox - a.x - projM * abx, mdy = oy - a.y - projM * aby, mdz = oz - a.z - projM * abz;
            ...
            if (A > 1e-8) {
                ...
                if (disc >= 0) {
                    ...
                    if (tc != null) {
                        ...
                        const s = ((px - a.x) * abx + (py - a.y) * aby + (pz - a.z) * abz) / ab2;
                        if (s >= 0 && s <= 1) tCyl = tc;
                    }
                }
            }
            ...
            if (tHit != null && tHit >= 0 && tHit <= maxDist) {
                hitDist = tHit;
                const px = ox + dx * tHit, py = oy + dy * tHit, pz = oz + dz * tHit;
                // normal
                let nx = 0, ny = 0, nz = 0;
                if (tHit === tCyl && tCyl != null) {
                    const s = ((px - a.x) * abx + (py - a.y) * aby + (pz - a.z) * abz) / ab2;
                    const qx = a.x + abx * s, qy = a.y + aby * s, qz = a.z + abz * s;
                    ...
                    if (tA != null && dA2 <= dB2) { nx = dAx; ny = dAy; nz = dAz; } else { nx = dBx; ny = dBy; nz = dBz; }
                }
