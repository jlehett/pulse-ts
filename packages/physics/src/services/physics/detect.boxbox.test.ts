...
describe('detectManifold box-box', () => {
    function makeBox(world: World, pos: Vec3, hx: number, hy: number, hz: number, rotZ = 0) {
        const n = new Node(); world.add(n);
        const t = attachComponent(n, Transform);
        t.localPosition.set(pos.x, pos.y, pos.z);
        if (rotZ !== 0) {
            const s = Math.sin(rotZ / 2), c = Math.cos(rotZ / 2);
            t.localRotation.set(0, 0, s, c).normalize();
        }
        const c = attachComponent(n, Collider); c.kind = 'box';
        c.halfX = hx; c.halfY = hy; c.halfZ = hz;
        return { node: n, t, c };
    }
    ...
    it('returns up to 4 contact points with correct normal orientation', () => {
        ...
        const dir = new Vec3(centerB.x - centerA.x, centerB.y - centerA.y, centerB.z - centerA.z);
        const n = pts[0]!;
