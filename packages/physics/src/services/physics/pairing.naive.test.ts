...
describe('pairing naive fallback', () => {
    it('returns all unique pairs when cellSize invalid', () => {
        ...
        const cols: Collider[] = [];
        for (let i = 0; i < 3; i++) {
            const n = new Node(); nodes.push(n); world.add(n);
            const t = attachComponent(n, Transform); t.localPosition.set(i * 10, 0, 0);
            const c = attachComponent(n, Collider); c.kind = 'sphere'; c.radius = 0.5; cols.push(c);
        }
