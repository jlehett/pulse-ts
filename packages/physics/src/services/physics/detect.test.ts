...
describe('detect and manifolds', () => {
    ...
    it('detects sphere-sphere overlap', () => {
        ...
        ca.kind = 'sphere'; ca.radius = 0.5;
        cb.kind = 'sphere'; cb.radius = 0.6;
    ...
    it('builds a manifold for box-plane with multiple points (<=4)', () => {
        const { ta, tb, ca, cb } = setupPair();
        ca.kind = 'box';
        ca.halfX = 0.5; ca.halfY = 0.5; ca.halfZ = 0.5;
        cb.kind = 'plane';
    ...
    it('builds a manifold for box-capsule via sampling', () => {
        const { ta, tb, ca, cb } = setupPair();
        ca.kind = 'box'; ca.halfX = 0.6; ca.halfY = 0.3; ca.halfZ = 0.4;
        cb.kind = 'capsule'; cb.capRadius = 0.25; cb.capHalfHeight = 0.4;
        ta.localPosition.set(0, 0, 0);
