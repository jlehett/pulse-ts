...
export function detectCollision(a: Collider, b: Collider): Contact {
    ...
    // plane combinations
    if (a.kind === 'sphere' && b.kind === 'plane') return spherePlane(ax, ay, az, a.radius, b);
    if (a.kind === 'plane' && b.kind === 'sphere') {
        const res = spherePlane(bx, by, bz, b.radius, a);
        return res ? { nx: -res.nx, ny: -res.ny, nz: -res.nz, depth: res.depth } : null;
    }
    ...
    // capsule combos
    if (a.kind === 'capsule' && b.kind === 'sphere') return capsuleSphere(a, { x: bx, y: by, z: bz }, b.radius);
...
export function detectManifold(a: Collider, b: Collider): ContactConstraint[] {
    ...
    if (a.kind === 'box' && b.kind === 'box') {
        ...
        const insideB = (p: Vec3) => {
            const v = new Vec3(p.x - fb.center.x, p.y - fb.center.y, p.z - fb.center.z);
            ...
            return Math.abs(x) <= fb.hx + eps && Math.abs(y) <= fb.hy + eps && Math.abs(z) <= fb.hz + eps;
        };
        const insideA = (p: Vec3) => {
            const v = new Vec3(p.x - fa.center.x, p.y - fa.center.y, p.z - fa.center.z);
            ...
            return Math.abs(x) <= fa.hx + eps && Math.abs(y) <= fa.hy + eps && Math.abs(z) <= fa.hz + eps;
        };
        const supportB = Math.abs(n.x * fb.U0.x + n.y * fb.U0.y + n.z * fb.U0.z) * fb.hx +
            Math.abs(n.x * fb.U1.x + n.y * fb.U1.y + n.z * fb.U1.z) * fb.hy +
            Math.abs(n.x * fb.U2.x + n.y * fb.U2.y + n.z * fb.U2.z) * fb.hz;
        const supportA = Math.abs(-n.x * fa.U0.x + -n.y * fa.U0.y + -n.z * fa.U0.z) * fa.hx +
        ...
        for (const v of vertsA) {
            ...
            const proj = (v.x - fb.center.x) * n.x + (v.y - fb.center.y) * n.y + (v.z - fb.center.z) * n.z;
            const depth = supportB - proj;
            if (depth > eps) constraints.push({ a, b, nx, ny, nz, depth, px: v.x, py: v.y, pz: v.z });
        }
        for (const v of vertsB) {
            if (!insideA(v)) continue;
            const proj = (v.x - fa.center.x) * (-n.x) + (v.y - fa.center.y) * (-n.y) + (v.z - fa.center.z) * (-n.z);
            const depth = supportA - proj;
            if (depth > eps) constraints.push({ a, b, nx, ny, nz, depth, px: v.x, py: v.y, pz: v.z });
        }
        if (constraints.length === 0) return [{ a, b, nx, ny, nz, depth: base.depth }];
    ...
    if (a.kind === 'box' && b.kind === 'plane') {
        ...
        for (const v of verts) {
            ...
            if (d < 0) constraints.push({ a, b, nx: -n.x, ny: -n.y, nz: -n.z, depth: -d, px: v.x, py: v.y, pz: v.z });
        }
    ...
    if (a.kind === 'capsule' && b.kind === 'plane') {
        ...
        if (dA < r) out.push({ a, b, nx: -n.x, ny: -n.y, nz: -n.z, depth: r - dA, px: pa.x - n.x * r, py: pa.y - n.y * r, pz: pa.z - n.z * r });
        if (dB < r) out.push({ a, b, nx: -n.x, ny: -n.y, nz: -n.z, depth: r - dB, px: pb.x - n.x * r, py: pb.y - n.y * r, pz: pb.z - n.z * r });
        if (out.length === 0) {
            // closest point on segment to plane
            // project pa and pb distances and clamp
            const t = (dA - Math.min(dA, dB)) / Math.max(1e-8, Math.abs(dA - dB));
            ...
            if (d < r) out.push({ a, b, nx: -n.x, ny: -n.y, nz: -n.z, depth: r - d, px: cx - n.x * r, py: cy - n.y * r, pz: cz - n.z * r });
        }
    ...
    if (a.kind === 'box' && b.kind === 'capsule') {
        ...
        const toLocal = (v: Vec3) => new Vec3(
            (v.x - fb.center.x) * fb.U0.x + (v.y - fb.center.y) * fb.U0.y + (v.z - fb.center.z) * fb.U0.z,
            (v.x - fb.center.x) * fb.U1.x + (v.y - fb.center.y) * fb.U1.y + (v.z - fb.center.z) * fb.U1.z,
            (v.x - fb.center.x) * fb.U2.x + (v.y - fb.center.y) * fb.U2.y + (v.z - fb.center.z) * fb.U2.z,
        );
        ...
        for (const s of sampleS) {
            ...
            const dx = pL.x - qx, dy = pL.y - qy, dz = pL.z - qz;
            ...
            if (d2 <= b.capRadius * b.capRadius) {
                ...
                out.push({ a, b, nx, ny, nz, depth: b.capRadius - d, px: qWorld.x, py: qWorld.y, pz: qWorld.z });
            }
        }
...
function planeWorld(c: Collider): { p: Vec3; n: Vec3 } {
    ...
    const n = Quat.rotateVector(trs.rotation, c.planeNormal.clone().normalize());
    const off = Quat.rotateVector(trs.rotation, c.offset.clone());
    const p = new Vec3(trs.position.x + off.x, trs.position.y + off.y, trs.position.z + off.z);
    return { p, n };
}

function spherePlane(sx: number, sy: number, sz: number, r: number, plane: Collider): Contact {
...
function obbPlane(box: Collider, plane: Collider): Contact {
    ...
    const { p, n } = planeWorld(plane);
    const extent = Math.abs(A0.x * n.x + A0.y * n.y + A0.z * n.z) * box.halfX +
...
function capsuleWorldSegment(c: Collider): { a: Vec3; b: Vec3; r: number } {
    ...
    const off = Quat.rotateVector(trs.rotation, c.offset.clone());
    const center = new Vec3(trs.position.x + off.x, trs.position.y + off.y, trs.position.z + off.z);
    const a = new Vec3(center.x + up.x * c.capHalfHeight, center.y + up.y * c.capHalfHeight, center.z + up.z * c.capHalfHeight);
    const b = new Vec3(center.x - up.x * c.capHalfHeight, center.y - up.y * c.capHalfHeight, center.z - up.z * c.capHalfHeight);
...
function capsuleSphere(cap: Collider, sphereCenter: { x: number; y: number; z: number }, sphereRadius: number): Contact {
...
function closestPointSegmentAABBLocal(a: Vec3, b: Vec3, hx: number, hy: number, hz: number): { p: Vec3; q: Vec3; s: number; dist2: number } {
    const d = new Vec3(b.x - a.x, b.y - a.y, b.z - a.z);
    const candidates: number[] = [0, 1];
    for (const [ai, di, h] of [[a.x, d.x, hx], [a.y, d.y, hy], [a.z, d.z, hz]] as Array<[number, number, number]>) {
        if (Math.abs(di) > 1e-8) {
            const s1 = (-h - ai) / di;
            const s2 = (h - ai) / di;
    ...
    let best = { p: a.clone(), q: new Vec3(clamp(a.x, -hx, hx), clamp(a.y, -hy, hy), clamp(a.z, -hz, hz)), s: 0, dist2: Infinity };
    candidates.sort((x, y) => x - y);
    for (let i = 0; i < candidates.length - 1; i++) {
        const sL = candidates[i]!; const sR = candidates[i + 1]!;
        const sm = (sL + sR) * 0.5;
        const testS = [sL, sm, sR];
        for (const s of testS) {
            const px = a.x + d.x * s, py = a.y + d.y * s, pz = a.z + d.z * s;
            const qx = clamp(px, -hx, hx), qy = clamp(py, -hy, hy), qz = clamp(pz, -hz, hz);
            const dx = px - qx, dy = py - qy, dz = pz - qz;
            const dist2 = dx * dx + dy * dy + dz * dz;
            if (dist2 < best.dist2) best = { p: new Vec3(px, py, pz), q: new Vec3(qx, qy, qz), s, dist2 };
        }
    }
    return best;
}

function capsuleObb(cap: Collider, box: Collider): Contact {
    const { center, U0, U1, U2, hx, hy, hz } = obbFrame(box);
    const seg = capsuleWorldSegment(cap);
    const toLocal = (v: Vec3) => new Vec3(
        (v.x - center.x) * U0.x + (v.y - center.y) * U0.y + (v.z - center.z) * U0.z,
        (v.x - center.x) * U1.x + (v.y - center.y) * U1.y + (v.z - center.z) * U1.z,
        (v.x - center.x) * U2.x + (v.y - center.y) * U2.y + (v.z - center.z) * U2.z,
    );
    ...
    const nL = new Vec3(best.p.x - best.q.x, best.p.y - best.q.y, best.p.z - best.q.z);
...
function capsuleCapsule(a: Collider, b: Collider): Contact {
    ...
    s = (aDotb * t - aDotr) / aLen2; s = Math.max(0, Math.min(1, s));
    const pa = new Vec3(sa.a.x + da.x * s, sa.a.y + da.y * s, sa.a.z + da.z * s);
    const pb = new Vec3(sb.a.x + db.x * t, sb.a.y + db.y * t, sb.a.z + db.z * t);
    const vx = pb.x - pa.x, vy = pb.y - pa.y, vz = pb.z - pa.z;
    const d2 = vx * vx + vy * vy + vz * vz;
