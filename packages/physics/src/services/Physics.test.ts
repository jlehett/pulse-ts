...
import { installPhysics, RigidBody, setLayer, setMask, addToMask, layer } from '..';
...
describe('PhysicsService integration', () => {
    ...
    it('raycast honors layer mask', () => {
        ...
        c1.kind = 'box';
        c1.halfX = 0.25; c1.halfY = 0.25; c1.halfZ = 0.25; // centered at x=0
        setLayer(c1, 1); // layer index 1
        const c2 = attachComponent(n2, Collider);
        c2.kind = 'box';
        c2.halfX = 0.25; c2.halfY = 0.25; c2.halfZ = 0.25; // centered at x=2
    ...
    it('raycasts against a plane and detects sphere-plane contact', () => {
        ...
        world.add(planeNode);
        const tp = attachComponent(planeNode, Transform);
    ...
    it('applies angular effects on tilted box impact with plane', () => {
        ...
        cb.kind = 'box';
        cb.halfX = 0.5; cb.halfY = 0.5; cb.halfZ = 0.5;

        // apply rotation so contacts are asymmetric
        const ang = 0.6;
        tb.localRotation.set(0, 0, Math.sin(ang / 2), Math.cos(ang / 2)).normalize();

        let wmag = 0;
        for (let i = 0; i < 10; i++) {
            physics.step(1 / 120);
            wmag = Math.hypot(rb.angularVelocity.x, rb.angularVelocity.y, rb.angularVelocity.z);
            if (wmag > 1e-6) break;
        }
