...
describe('materials helpers', () => {
    const mkBody = (e?: number, f?: number) =>
        ({ restitution: e, friction: f }) as any;
    const mkCol = (e?: number, f?: number) =>
        ({ restitution: e, friction: f }) as any;
    ...
    it('getRestitution and getFriction use max(body, collider) or fallback', () => {
        ...
        expect(
            getRestitution(mkBody(undefined), mkCol(undefined), 0.25),
        ).toBeCloseTo(0.25);
        expect(
            getFriction(mkBody(undefined, 0.3), mkCol(undefined, 0.8)),
        ).toBeCloseTo(0.8);
    });
