...
import { combineFriction, combineRestitution } from '../../materials/materials';
...
export function solveContactsIterative(
    ...
    for (let iter = 0; iter < iterations; iter++) {
        for (const c of constraints) {
            ...
            const kNormal =
                totalInv +
                (rnA.x * Ia_rnA.x + rnA.y * Ia_rnA.y + rnA.z * Ia_rnA.z) +
                (rnB.x * Ib_rnB.x + rnB.y * Ib_rnB.y + rnB.z * Ib_rnB.z);
            ...
            if (vrel - biasVel < 0) {
                const e = combineRestitution(rbA, a, rbB, b);
                const jn =
                    -((1 + e) * vrel + biasVel) / Math.max(1e-8, kNormal);
                const jx = nx * jn,
                    jy = ny * jn,
                    jz = nz * jn;
                if (invMassA > 0) {
                ...
                const rXJ_a = new Vec3(
                    ra.y * jz - ra.z * jy,
                    ra.z * jx - ra.x * jz,
                    ra.x * jy - ra.y * jx,
                );
                const rXJ_b = new Vec3(
                    rbv.y * jz - rbv.z * jy,
                    rbv.z * jx - rbv.x * jz,
                    rbv.x * jy - rbv.y * jx,
                );
                ...
                if (tmag > 1e-8) {
                    ...
                    const rXJt_a = new Vec3(
                        ra.y * jtz - ra.z * jty,
                        ra.z * jtx - ra.x * jtz,
                        ra.x * jty - ra.y * jtx,
                    );
                    const rXJt_b = new Vec3(
                        rbv.y * jtz - rbv.z * jty,
                        rbv.z * jtx - rbv.x * jtz,
                        rbv.x * jty - rbv.y * jtx,
                    );
...
function invInertiaApply(
    rb: RigidBody | undefined | null,
    v: Vec3,
    t: Transform,
): Vec3 {
