...
function makeBodies(overlapDepth = 0.02) {
    ...
    ca.kind = 'box';
    ca.halfX = ca.halfY = ca.halfZ = 0.5;
    const cb = attachComponent(b, Collider);
    cb.kind = 'box';
    cb.halfX = cb.halfY = cb.halfZ = 0.5;
...
describe('solver unit', () => {
    it('correctPositions uses slop and splits correction based on mass', () => {
        ...
        const yA0 = ta.localPosition.y,
            yB0 = tb.localPosition.y;
    ...
    it('solveContactsIterative reduces approach velocity along normal', () => {
        ...
        const vrel0 =
            (rbB.linearVelocity.y - rbA.linearVelocity.y) * constraint.ny;
    ...
    it('applies angular impulses when contact point has an offset', () => {
        ...
        const wmagA = Math.hypot(
            rbA.angularVelocity.x,
            rbA.angularVelocity.y,
            rbA.angularVelocity.z,
        );
        const wmagB = Math.hypot(
            rbB.angularVelocity.x,
            rbB.angularVelocity.y,
            rbB.angularVelocity.z,
        );
