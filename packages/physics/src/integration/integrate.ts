...
import { refreshAutomaticInertia } from '../internal/inertia';

/**
 * Applies accumulated forces/impulses and gravity to body velocities; clears accumulators.
 */
export function integrateVelocities(bodies: Iterable<RigidBody>, gravity: Vec3, dt: number, hasCollider: (c: Collider) => boolean): void {
    for (const rb of bodies) {
        if (!rb.owner?.world) continue;
        if (rb.autoComputeInertia) refreshAutomaticInertia(rb, hasCollider);

        const hasImpulse = rb.impulse.x !== 0 || rb.impulse.y !== 0 || rb.impulse.z !== 0;
        const hasForce = rb.force.x !== 0 || rb.force.y !== 0 || rb.force.z !== 0;
        const hasAngularImpulse = rb.angularImpulse.x !== 0 || rb.angularImpulse.y !== 0 || rb.angularImpulse.z !== 0;
        const hasTorque = rb.torque.x !== 0 || rb.torque.y !== 0 || rb.torque.z !== 0;
        if (rb.type !== 'dynamic') {
        ...
        if (rb.linearDamping > 0) {
            ...
            rb.linearVelocity.x *= f; rb.linearVelocity.y *= f; rb.linearVelocity.z *= f;
        }
        if (rb.angularDamping > 0) {
            const f = Math.max(0, 1 - rb.angularDamping * dt);
            rb.angularVelocity.x *= f; rb.angularVelocity.y *= f; rb.angularVelocity.z *= f;
        }
    }
}

/**
 * Integrates body velocities into transforms, applies optional world plane constraint, and updates rotations.
 */
export function integrateTransforms(bodies: Iterable<RigidBody>, dt: number, planeY?: number): void {
    ...
    for (const rb of bodies) {
        ...
        if (rb.type === 'dynamic') {
            ...
            const wx = rb.angularVelocity.x, wy = rb.angularVelocity.y, wz = rb.angularVelocity.z;
            if (wx !== 0 || wy !== 0 || wz !== 0) {
                ...
                const delta = tmpQuat.set(wx * scale, wy * scale, wz * scale, cosHalf).normalize();
