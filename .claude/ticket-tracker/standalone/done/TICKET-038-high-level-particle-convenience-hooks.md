...
- **2026-02-26**: Implemented `ParticlesService`, `installParticles()`, `useParticleBurst()`, `useParticleEmitter()`. Migrated platformer demo (LevelNode, CollectibleNode, EnemyNode). Removed `ParticleEffectsCtx` context and `config/particles.ts`. 102 tests pass. Merged via PR #46.
